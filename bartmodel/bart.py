...
class CaGFBartDecoder(FBartDecoder):
    ...
    def forward(self, tokens, state):
        ...
        # 首先计算的是
        eos_scores = F.linear(hidden_state, self.dropout_layer(self.decoder.embed_tokens.weight[102:103]))  # bsz x max_len x 1
        ...
        mask = mask.__or__(src_tokens.eq(102).cumsum(dim=1).ge(1).unsqueeze(1))
        word_scores = word_scores.masked_fill(mask, -1e32)
...
class BartSeq2SeqModel(Seq2SeqModel):
    @classmethod
    def build_model(cls, bart_model, tokenizer, label_ids, mapping2id, decoder_type=None,
        ...
        for token in mapping2id:
            index = mapping2id[token]
            embed = model.encoder.embed_tokens.weight.data[index]
        ...
        if decoder_type is None:
            ...
            decoder = CaGFBartDecoder(decoder, pad_token_id=0, label_ids=label_ids,
